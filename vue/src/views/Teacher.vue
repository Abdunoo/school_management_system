...
<template>
    <div class="bg-white rounded-lg shadow p-6 space-y-6 h-full">
        <h1 class="text-xl font-bold text-secondary">{{ pageTitle }}</h1>
            ...
            <div class="flex w-full justify-between space-x-6">
                ...
                <div class="relative flex space-x-2 text-secondary">
                    ...
                    <input type="text" v-model="searchQuery" placeholder="Search..."
                        class="block w-full text-sm border border-gray-300 rounded-md shadow-sm focus:ring-primary focus:border-primary px-3 py-2 placeholder-gray-400 focus:outline-none">
        ...
        <div class="overflow-y-auto rounded-xl border border-gray-300 bg-gray-50 ">
            <table class="min-w-full table-auto text-sm text-left space-y-8">
                <thead>
                    <tr class="bg-gray-200 grid grid-cols-7">
                        <th class="px-4 py-3 text-secondary">NIP</th>
                        <th class="px-4 py-3 text-secondary">Spesialisasi</th>
        ...
        <div class="overflow-y-auto max-h-full h-3/5 rounded-xl border border-gray-300 bg-gray-50 ">
            <table class="min-w-full table-auto text-sm text-left space-y-8">
                <tbody class="divide-y divide-gray-200">
                    <tr v-for="teacher in teachers" :key="teacher.id"
                        class="hover:bg-gray-100 transition grid grid-cols-7">
                        ...
                        <td class="px-4 py-3 text-secondary">
                            <Button> {{ teacher.user.is_active ? 'Aktif' : 'Tidak Aktif' }}</Button>
                        </td>
        ...
        <div class="flex justify-between items-center">
            ...
            <button @click="currentPage++" class="bg-gray-200 px-4 py-2 rounded">Next</button>
        </div>
    </div>
</template>